...
-- former sv_mapvote.lua	
	...
	function GM:ChangeMapTo(map)
		...
		print("[ultimateph] Rotate changing map to " .. map)
	...
	function GM:SaveMapList()
		...
		if !file.Exists("ultimateph/", "DATA") then
			file.CreateDir("ultimateph")
		end
		...
		file.Write("ultimateph/maplist.txt", txt)
	end
	
	function GM:LoadMapList()
		local jason = file.ReadDataAndContent("ultimateph/maplist.txt")
		if jason then
			local tbl = {}
